...
from TreeWidthTreeContainment import BruteForce
from TreeWidthTreeContainment import TreeDecomposition
import sys
sys.path.append('../')
from utils import *
...
def tc_brute_force(tree_in, network_in, add_dummy=True, return_mapping=False):
    if add_dummy:
        root_nw = None
        for x in network_in.nodes:
            if network_in.in_degree[x] == 0:
                root_nw = x
        network_in.add_edge(10**6,root_nw)
    ...
    while todo_list:
        ...
        if node is None:
            ...
            if tcn_contains(network, tree):
                #assert utils.is_tree_child(network_copy)
                #if return_mapping:
                #    return True, network_copy
                return True
            ...
            for parent in network.predecessors(node):
                ...
                if copy_network.out_degree(parent) == 1:
                    ...
                    while fix_list:
                        ...
                        if fix_node in copy_network and copy_network.out_degree(fix_node) == 1:
